...
[^*]: Эти поля *могли бы* быть определены внутри макроса, но тогда они бы явно передавались в каждом вызове (из-за гигиены макроса).  Чтобы быть честным, к тому временя как я понял, что мне *нужно* их определить, макрос был уже почти написан и ... ну, *вы* бы хотели заново пройти весь путь и исправить это, если на самому деле *огромной нужды* в этом нет?
...
Следующее, у нас есть одно правило для *почти* каждого кода операции. Для этого мы берем код операции, подставляем соответствующй код на Rust, затем рекурсивно перемещаемся в хвост входа: прямо по книге [TT muncher](../pat/README.html#incremental-tt-munchers).
...
...переполнением стека и падением. И по настоящий день, эзоязык-как-макрос остается решительно *нежизнеспособным* методом разработки на Rust.
