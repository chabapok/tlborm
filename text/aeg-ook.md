...
Ввычисления в языке организованы очень просто: память представляет собой массив
...
значения по умолчанию ... [попробуйте угадать](https://en.wikipedia.org/wiki/Hello_world_program).
...
Эти поля нужно разместить здесь, чтобы убедиться, что они видимы для
развертывания макроса.[^*]

[^*]: Эти поля *могли бы* быть определены внутри макроса, но тогда их бы 
...
выход, массив памяти *и т.д.*. Мы используем [связывание ТТ](../pat/README.html
#tt-bundling) для передачи этих символов дальше, промежуточным правилам.
...
перемещаемся в конец входа: раздел [последовательный потребитель TT](../pat/README.html
...
поздно, а закончить его *вовремя*.[^вовремя]

[^вовремя]:
    Известный факт[^факт], что сказка "Маша и три медведя" - это на самом 
...
Manish Goregaokar затем [реализовал интерпретатор Brainfuck, используя макрос Ho
dor!](https://www.reddit.com/r/rust/comments/39wvrm/hodor_esolang_as_a_rust_macr
o/cs76rqk?context=10000). Таким образом, это интерпретатор  Brainfuck,
написанный на Hodor!, который, в свою очередь, написан на  `macro_rules!`.
